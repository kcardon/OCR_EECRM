...
class EmployeePermission(BasePermission):
    ...
    def can_update_employee(self, request):
        return bool(
            ...
            and self.get_user_group(request) in ["Management", "Sales"]
        )

    def can_create_employee(self, request):
        return (
            request.user.is_authenticated
            and self.get_user_group(request) == "Management"
        )
...
class ClientPermission(BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        if self.get_user_group(request) == "Management":
            ...
            clients = Event.objects.filter(support_contact=request.user).values_list(
                "client", flat=True
            )
            return obj in clients
